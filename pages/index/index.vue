  ...
  <view class="ph-container">
    ...
    <view class="ph-header">
      ...
      <view class="ph-header-right">
        ...
        <view class="ph-icon ph-user-icon">
          <text class="ph-icon-text">👤</text>
        </view>
      </view>
    </view>
    ...
    <view class="ph-sidebar" :class="{ 'active': sidebarOpen }">
      ...
      <view class="ph-sidebar-menu">
        ...
        <view class="ph-menu-item">
          <view class="ph-menu-icon">▶</view>
          <view class="ph-menu-text">短片</view>
          <view class="ph-menu-badge">BETA</view>
        </view>
        
        <view class="ph-menu-item">
          <view class="ph-menu-icon">👍</view>
          <view class="ph-menu-text">推荐视频</view>
        </view>
    ...
    <scroll-view scroll-y="true" class="ph-content">
      ...
      <!-- 分类导航 -->
      <scroll-view scroll-x="true" class="ph-categories">
        <view class="ph-category-item active">
          <text>推荐</text>
        </view>
        <view class="ph-category-item">
          <text>热门</text>
        </view>
        <view class="ph-category-item">
          <text>最新</text>
      ...
      <view class="ph-video-grid">
        <view class="ph-video-item" v-for="(item, index) in hotVideos" :key="index">
          <view class="ph-video-cover">
            <image 
              :src="`https://placehold.co/400x225/333/ffffff?text=Video+${index+1}`" 
              mode="aspectFill"
            ></image>
      ...
      <view class="ph-video-grid">
        <view class="ph-video-item" v-for="(item, index) in newVideos" :key="index">
          <view class="ph-video-cover">
            <image 
              :src="`https://placehold.co/400x225/333/ffffff?text=New+${index+1}`" 
              mode="aspectFill"
            ></image>
            <view class="ph-video-duration">{{ item.duration }}</view>
            <view class="ph-video-hd" v-if="item.hd">HD</view>
      ...
      <view class="ph-category-grid">
        <view class="ph-category-card" v-for="(item, index) in categories" :key="index">
          <image 
            :src="`https://placehold.co/200x120/333/ffffff?text=Category+${index+1}`" 
            mode="aspectFill"
          ></image>
      ...
      <view class="ph-model-list">
        <view class="ph-model-item" v-for="(item, index) in models" :key="index">
          <view class="ph-model-avatar">
            <image 
              :src="`https://placehold.co/100x100/333/ffffff?text=Channel+${index+1}`" 
              mode="aspectFill"
            ></image>
          </view>
...
// 热门模特数据
const models = ref([
  { name: '频道名称1', videos: '215' },
  { name: '频道名称2', videos: '186' },
  { name: '频道名称3', videos: '147' },
  { name: '频道名称4', videos: '132' }
]);
</script>